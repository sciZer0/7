...
Fundamental Problem 6 is that on each of the existing blockchains, there are low incentives for its validators and developers if calculated per person — meaning unclear prospects for the blockchain's sustainability and software development in the long term.

Ideally, a blockchain must always create highly-generous incentives for its validators and developers when calculated per person — in order to be a more sustainable working system and have strong grounds for constant software development in the long term.
...
Without the ideal stablecoin, plus having all these fundamental problems above, getting mass adoption for crypto in general is extremely-hard.
...
From day one, the Vote blockchain has the ideal stablecoin — a really decentralized, permissionless, trustless, ultra-stable and importantly, safe stablecoin — meaning, among other things, which never relies on banks or multi-signatures, and where state and logic of the dApp aren't in the same place.
...
As for literally transferring value, the Bitcoin token might have a better life — not the leadership but just a better life — only as a native, wrapped token on an advanced blockchain — bridged, of course, through a native trustless bridge. 
