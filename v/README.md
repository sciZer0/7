...
Including, in order for the Bitcoin blockchain to get started, the creator at the start had to use the extremely-reliable incentive system, implementing the unproportionally big rewards for the most earliest adopters (miners) and predefining the awful Gini — and also to use the extremely-reliable but extremely-inefficient consensus.
...
Ideally, a blockchain must create extremely-generous long-term incentives for validators and developers when calculated per person — in order to always be able to attract highly-professional developers, and as a result to always be a more sustainable working system.
The Vote blockchain ideally solves this problem from day one.
