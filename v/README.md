...
The only way for a blockchain to be highly-sustainable (really-unstoppable) and permissionless is to simultaneously use the public-private key pair authentication mechanism and be a people-ruled decentralized system – without being under control of any single entity.
...
By the way, total immutability is especially not good for smart-contracts being built on and sometimes connected to changeable blockchains since if any bugs are discovered in the immutable code, they cannot be fixed and users can lose all of their funds. But, on the other hand, changeability of smart-contracts inevitably involves, among other things, multisigs of few people  that causes already other problems. *A little more on problems of smart-contracts and solutions a bit below.*
...
Of course, for now the Bitcoin blockchain is both permissionless and highly-sustainable. But, it might have been highly-sustainable for a while until some moment — for example, until there is a token with pre-programmed decreasing of supply.
